...
router.post('/register', async (req, res, next) => {
  try {
    const { eventId, name, description, logo, website, contactEmail, contactPhone, attendanceType, staff } = req.body;
...
router.put('/:id', protect, authorize('super_admin', 'organizer', 'event_manager'), async (req, res, next) => {
  try {
    ...
    if (contactEmail) exhibitor.contactEmail = contactEmail.toLowerCase();
    if (contactPhone) exhibitor.contactPhone = contactPhone;
    if (boothNumber !== undefined) exhibitor.boothNumber = boothNumber;
